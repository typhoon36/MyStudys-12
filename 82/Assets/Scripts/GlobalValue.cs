...
{
    public int m_DestExp = 0; //다음 레벨까지 필요한 경험치 
...
public class GlobalValue
{
    ...
    public static List<LevelTable> g_LevelTable = new List<LevelTable>(); //레벨 테이블

    public static string g_NickName = "";   //유저의 별명
    public static int g_BestScore = 0;      //최고 점수
    public static int g_UserGold = 0;      //보유 게임 머니
    public static int g_Exp = 0;         //현재 경험치
    public static int g_Level = 1;       //현재 레벨
    ...
    public static void LoadGameData()
    {
        ...
        if (g_CurSkillCount.Count <= 0)
        {
            ...
            for (int i = 0; i < (int)SkillType.SkCount; i++)
            {
                //로컬 로딩은 제거 //a_SkCount = PlayerPrefs.GetInt($"Skill_Item_{i}", 1);
                a_SkCount = 0;
        ...
        //--- 레벨 테이블 로딩(Json 파일 로딩)
        if (g_LevelTable.Count <= 0)
        {
            TextAsset JsonData = Resources.Load<TextAsset>("LevelTable");
            string StrJsonData = JsonData.text;
            var a_ParseData = JSON.Parse(StrJsonData);

            if (a_ParseData["LvTable"] != null)
                for (int i = 0; i < a_ParseData["LvTable"].Count; i++)
                {
                    int a_DestExp = a_ParseData["LvTable"][i].AsInt;
                    LevelTable a_Nd = new LevelTable();
                    a_Nd.m_DestExp = a_DestExp;
                    g_LevelTable.Add(a_Nd);
