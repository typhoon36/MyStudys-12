...
{
    public int m_DestExp = 0;   //다음 레벨로 가기 위한 경험치 Destination Experience
...
public class GlobalValue 
{
    ...
    public static void LoadGameData()
    {
        ...
        //--- 레벨, 경험치 테이블 Json 로딩
        if(g_LvTable.Count <= 0)
        {
            TextAsset JsonData = Resources.Load<TextAsset>("LevelTable");
            string strJsonData = JsonData.text;
            var a_ParseJs = JSON.Parse(strJsonData);

            if (a_ParseJs["LvTable"] != null)
            for(int i  = 0; i < a_ParseJs["LvTable"].Count; i++)
            {
                int a_DestExp = a_ParseJs["LvTable"][i].AsInt;
                LevelTable a_Node = new LevelTable();
                a_Node.m_DestExp = a_DestExp;
